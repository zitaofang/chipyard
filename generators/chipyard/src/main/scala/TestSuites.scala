...
import biriscv.{BiRISCVTilesKey}
...
object TestSuiteHelper
{
  ...
  */
  ...
  /**
  * Add BiRISCV tests (asm, bmark, regression)
  */
  def addBiRISCVTestSuites(implicit p: Parameters) = {
    val xlen = p(XLen)
    p(BiRISCVTilesKey).find(_.hartId == 0).map { tileParams =>
      val coreParams = tileParams.core
      val vm = coreParams.useVM
      val env = if (vm) List("p","v") else List("p")
      coreParams.fpu foreach { case cfg =>
